...
OUT_DIR      = os.path.join(PROJECT_ROOT, 'notebooks', 'objective_1', 'output_data', 'step1_output_threshold')
...
EXCLUDE_COLS = ['filename', 'Numeric numbers', 'image_path',
                'image_no', 'soil_type','soc','moisture','Sample_No']
...
# ============================================================
# RESTRICTED SOC MODELING DOMAIN
# ============================================================
...
def apply_soc_domain_filter(df):

    print("\n" + "=" * 65)
    print("SOC DOMAIN RESTRICTION")
    print("=" * 65)

    df = df.copy()

    df["soc"] = pd.to_numeric(
        df["soc"],
        errors="coerce"
    )
    ...
    print(f"  SOC modeling range : <= {SOC_MAX}")
    print(f"  Original images    : {len(df)}")
    print(f"  Images above cutoff: {len(excluded)}")
    print(f"  Images retained    : {len(retained)}")

    if len(excluded) > 0:

        print("\n  Excluded SOC distribution:")
    ...
    retained.to_csv(
        os.path.join(
            OUT_DIR,
            "soc_le_3_filtered.csv"
        ),
        index=False
    )

    print("\n  Saved: soc_above_3_excluded.csv")
    print("  Saved: soc_le_3_filtered.csv")

    # Safety checks
    if (retained["soc"] > SOC_MAX).any():
        raise ValueError(
            "SOC values above threshold still exist."
        )

    print(
        f"  Verification passed: "
        f"all retained SOC <= {SOC_MAX}"
    )
...
def main():
    ...
    print("\n" + "=" * 65)
    print("SOC PIPELINE — RESTRICTED DOMAIN")
    print("SOC <= 3.0")
    print("=" * 65)
    ...
    # Apply SOC cutoff FIRST
    merged, soc_excluded = apply_soc_domain_filter(
        merged_original
    )

    # Mahalanobis is calculated only within SOC <= 3 dataset
    (
        df_clean,
        df_outliers,
        distances,
        threshold,
        feature_cols
    ) = mahalanobis_outlier_removal(
        merged
    )

    # Image → physical sample mapping
    mapping = build_image_sample_mapping()

    # Grouped split
    df_calib, df_valid = grouped_sample_split(
        df_clean,
        mapping
    )
    ...
    print("\n" + "=" * 65)
    print("RESTRICTED SOC ANALYSIS COMPLETE")
    print("=" * 65)
    ...
    print(
        f"SOC > {SOC_MAX} excluded  : "
        f"{len(soc_excluded)}"
    )

    print(
        f"SOC <= {SOC_MAX} retained : "
        f"{len(merged)}"
    )
