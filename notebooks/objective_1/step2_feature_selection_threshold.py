...
"""
STEP 2 — DEFENSIBLE FEATURE SELECTION (THRESHOLD: SOC > 3 dropped)
=======================================================================
...
CALIB_CSV = os.path.join(PROJECT_ROOT, "notebooks", "objective_1",
    "output_data", "step1_output_threshold", "calibration_set.csv")
VALID_CSV  = os.path.join(PROJECT_ROOT, "notebooks", "objective_1",
    "output_data", "step1_output_threshold", "validation_set.csv")
OUT_DIR    = os.path.join(PROJECT_ROOT, "notebooks", "objective_1",
    "output_data", "step2_output_threshold")
...
def main():
    print("\n" + "="*70)
    print("STEP 2 — FEATURE SELECTION (THRESHOLD: SOC > 3 dropped)")
    ...
    # Apply SOC threshold — drop samples with SOC > 3
    before = len(df_cal)
    df_cal = df_cal[df_cal[TARGET] <= 3.0].reset_index(drop=True)
    df_val = df_val[df_val[TARGET] <= 3.0].reset_index(drop=True)
    print(f'  After SOC threshold (<=3): cal={len(df_cal)} images (removed {before-len(df_cal)})')
    print(f'  Validation after threshold: {len(df_val)} images')
    # Stage 1
