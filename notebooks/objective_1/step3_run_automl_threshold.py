...
"""
Step 3 - AutoML Grouped Validation (THRESHOLD: SOC > 3 dropped)
================================================================
FIXES APPLIED:
  ...
  3. GroupKFold correctly applied at image level
...
CALIB_CSV = os.path.join(PROJECT_ROOT, 'notebooks', 'objective_1', 'output_data',
                          'step2_output_threshold', 'calibration_selected_features.csv')
VALID_CSV  = os.path.join(PROJECT_ROOT, 'notebooks', 'objective_1', 'output_data',
                          'step2_output_threshold', 'validation_selected_features.csv')
...
TARGET_COL = 'soc'
GROUP_COL  = 'Sample_No'
SOC_THRESHOLD = 3.0  # Samples with SOC > 3 were dropped in Step 1

NON_PREDICTOR_COLS = ['image_no', 'moisture', 'soil_type', 'Sample_No']

print("\n" + "="*70)
print(f"AUTOML - GROUPED VALIDATION (SOC threshold: dropped SOC > {SOC_THRESHOLD})")
print("="*70)
...
print(f"  Calibration SOC range: {train_full[TARGET_COL].min():.2f} - {train_full[TARGET_COL].max():.2f}")
print(f"  Validation  SOC range: {test_full[TARGET_COL].min():.2f} - {test_full[TARGET_COL].max():.2f}")
    ...
    for fold_idx, (tr_idx, tu_idx) in enumerate(gkf.split(train_df, groups=groups), 1):
        ...
        try:
            ...
            for _, row in lb.iterrows():
                fold_results.setdefault(row['model'], []).append(-row['score_val'])
        except Exception as e:
...
q75, q25 = np.percentile(y_test, [75,25])
rpiq = (q75-q25)/rmse if rmse>0 else 0
comp = composite_score_metric(y_test, y_pred)
...
if GROUP_COL in test_full.columns:
    ...
    samp.to_csv(os.path.join(OUT_DIR, 'sample_predictions.csv'), index=False)

# Leaderboard
lb = predictor.leaderboard(test_df, silent=True)
print("\n  Model leaderboard:")
print(lb[['model','score_val']].head(10).to_string(index=False))
...
lb.to_csv(os.path.join(OUT_DIR, 'model_leaderboard.csv'), index=False)
pd.DataFrame([{'R2':r2,'RMSE':rmse,'RPD':rpd,'RPIQ':rpiq,'MAE':mae,'Composite':comp}]).to_csv(
